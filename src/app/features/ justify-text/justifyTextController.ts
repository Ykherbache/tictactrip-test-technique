...
import { JUSTIFY_TEXT_ERROR, JustifyTextError } from './errors/justifyTextError';
  ...
  public justify: MiddlewareFunction = async (req, res) => {
    try {
      ...
      if (typeof text !== 'string') {
        return res.status(400).send('Le corps de la requête doit être du texte brut');
      }
  ...
  private handleJustifyTextError(error: JustifyTextError, res: Response) {
    switch (error) {
      ...
      case JUSTIFY_TEXT_ERROR.INVALID_INPUT:
        return res.status(400).send('Le corps de la requête doit être du texte brut valide');
